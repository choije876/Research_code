...
start_time = datetime(2020, 8, 18, 3, 0) 
...
for idx, position in enumerate(region_points):
    ...
    fig, axes = plt.subplots(1, 3, figsize=(12, 3), gridspec_kw={'hspace': 0.22, 'wspace': 0.4}) 

    # Panel 1: RH ----- 
    ax0 = axes[0]

    ax0.plot(time, rh_ct_mean, label='CNTL', color='#6F8FAF', linewidth=liw, linestyle='-')
    ...
    ax0.set_xlabel('Time [LST]',fontsize=fts)
    ...
    ax0.set_ylim([85, 103])
    ax0.axhline(y=100, color='grey', linestyle='dotted', linewidth=1.4) 
    ...
    ax1.set_xlabel('Time [LST]',fontsize=fts)
    ax1.set_xticks(tick_indices)
    ax1.set_xticklabels(time_labels)

    ax1.tick_params(axis='y', labelsize=fts, width=2)
    ax1.set_ylabel('Latent Heat flux [W/m$^2$]', fontsize=fts)
    ax1.set_ylim([-4,55])
    ax1.axhline(y=0, color='grey', linestyle='dotted', linewidth=1.6)
    ...
    ax2.set_xlabel('Time [LST]',fontsize=fts)
    ax2.set_xticks(tick_indices)
    ax2.set_xticklabels(time_labels)

    ax2.tick_params(axis='y', labelsize=fts, width=2)
    ax2.set_ylabel('Sensible Heat Flux [W/m$^2$]', fontsize=fts)
    ax2.set_ylim([-15,20])
